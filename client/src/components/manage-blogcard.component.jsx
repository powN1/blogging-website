...
export const ManagePublishedBlogCard = ({ blog }) => {
	const { publishedAt, title, banner, blog_id, activity } = blog;
			...
			<div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
				...
				<div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
					...
					<div className="flex gap-6 mt-3">
						...
						<button className="lg:hidden pr-4 py-2 underline" onClick={() => setshowStats(prevVal => !prevVal)}>
							Stats
						</button>
...
const deleteBlog = (blog, access_token, target) => {
	...
	axios
		.post(
			import.meta.env.VITE_SERVER_DOMAIN + "/api/delete-blog",
			{ blog_id },
			{
				headers: {
		...
		.then(({ data }) => {
			...
			setStateFunc(prevVal => {
				...
				console.log({ ...prevVal, totalDocs: totalDocs - 1, deletedDocCount: deletedDocCount + 1 });
