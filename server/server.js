...
import serviceAccountKey from "./react-blog-website-71d78-firebase-adminsdk-no9f5-1fed842c56.json" assert { type: "json" };
//aws
...
server.post("/all-latest-blogs-count", (_req, res) => {
...
server.get("/get-upload-url", (_req, res) => {
  generateUploadUrl()
...
server.get("/trending-blogs", (_req, res) => {
  Blog.find({ draft: false })
    .populate("author", "personal_info.profile_img personal_info.fullname personal_info.username -_id")
    .sort({
    ...
    .then((comment) => {
      if (comment.parent) {
        Comment.findOneAndUpdate({ _id: comment.parent }, { $pull: { children: _id } })
          .then((_data) => console.log("comment deleted"))
          .catch((err) => console.log(err));
      }

      Notification.findOneAndDelete({ comment: _id })
        .then((_notification) => console.log("comment notification deleted"))
        .catch((err) => console.log(err));

      Notification.findOneAndDelete({ reply: _id })
        .then((_notification) => console.log("reply deleted"))
        .catch((_err) => console.log(ell));
  ...
  try {
    for (let i = 0; i < socialLinksArray.length; i++) {
      if (social_links[socialLinksArray[i]].length) {
        ...
        const updatedObj = { "personal_info.username": username, "personal_info.bio": bio, social_links };
    ...
    // console.log(err);
    return res.status(500).json({ error: "You must provide social links with http(s) included" + err });
  }
});
